	...
	switch (response_id) {
		...
		case FINDNEXT:
			...
			if (!gtk_text_buffer_get_selection_bounds((GtkTextBuffer*)page->buffer, &page->match_start, &page->match_end)) {
				gtk_text_buffer_get_iter_at_mark(
                    ...
                    &page->iter,
                    gtk_text_buffer_get_insert((GtkTextBuffer*)page->buffer)
                );
            }
		...
		case REPLACE:
			if (replaceAll == true) {
			    ...
			    do {
					if (gtk_text_buffer_get_selection_bounds((GtkTextBuffer*)page->buffer, &page->match_start, &page->match_end)) {
                        selectedtext = gtk_text_buffer_get_text(
                            (GtkTextBuffer*)page->buffer,
                            &page->match_start,
                            &page->match_end,
                            false
                        );
						if ((insensitiveSearch == true  && strcasecmp(selectedtext, searchtext) == 0) ||
                            (insensitiveSearch == false && strcmp(selectedtext, searchtext) == 0)) {
                        ...
                        if (selectedtext != NULL) g_free(selectedtext);
                    }
				}
...
void doSearchPrefs(GtkWidget* widget, gpointer data) {
	...
	switch ((long)data) {
		...
		case 3:
			...
			if(replaceAll == false)
				 gtk_button_set_label((GtkButton*)button, _("Replace"));
			else gtk_button_set_label((GtkButton*)button, _("Replace All"));
		break;
	}
}
